...
mapLoop:
    ...
    slli t3, t0, 2      # multiply to by 4, storing result in t0
    add t1, t1, t3      # offset the array address by the count * 4
    lw a0, 0(t1)        # load the value at that address into a0

    addi sp, sp, -12    # calling convention, maintain the temporary register.
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp)
    ...
    lw t2, 8(sp)        # calling convention, restore the temporary register.
    lw t1, 4(sp)
    lw t0, 0(sp)
    addi sp, sp, 12
