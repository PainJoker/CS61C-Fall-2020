...
class LogisimTest:
    ...
    def __call__(self, filename):
        ...
        proc = subprocess.Popen(
            ["java", "-jar", logisim_location, "-tty", "table", self.circ_path],
            cwd=self.group_path,
            stdin=stdinf,
...
def run_test(group_path, test_slug, output_type=None):
    ...
    reference_output_path = os.path.join(
        group_path, "reference_output/%s-ref.out" % output_slug
    )
    student_output_path = os.path.join(
        group_path, "student_output/%s-student.out" % output_slug
    )
...
def run_tests(mpart, mgroup, test):
    ...
    parts = (
        sorted(
            [
                folder
                for folder in os.listdir(os.path.join(script_dir, "tests"))
                if os.path.isdir(os.path.join(script_dir, "tests", folder))
            ]
        )
        if mpart is None
        else [mpart]
        ...
        groups = (
            ...
            if mgroup is not None
                ...
                [
                    ...
                    if os.path.isdir(os.path.join(part_path, folder))
                ]
            )
        )
        for group in groups:
            ...
            if not os.path.isdir(group_path):
                ...
                raise ValueError(
                    "Invalid test group: %s (choose from %s)"
                    % (group, ", ".join(groups))
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "part",
        choices=parts,
        help="The project part the test is under (a/b)",
        nargs="?",
        default=None,
    )
    parser.add_argument(
        "group",
        help="The group of tests to run. If left blank, it will run all the tests of the part",
        default=None,
        nargs="?",
    )
    parser.add_argument(
        "test",
        help="A specific test file to run. You must specify the correct group and the full filename! If left blank, it will be ignored",
        nargs="?",
    )
